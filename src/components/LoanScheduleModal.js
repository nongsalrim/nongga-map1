 ...
 * @description 💳 대출 현황 & 26개년 연도별 상환계획 정밀 명세서 모달 및 엑셀 다운로드 엔진 (천단위 구분기호 & 엑셀 스타일 서식 내장)
 */
...
export async function exportLoanExcel(loansList, farmName = '농가') {
  ...
  if (XLSX) {
    ...
    sheetData.push([
      '합계',
      '',
      formatMoney(scheduleInfo.totalAmount),
      '', '', '', '', ''
    ]);
    ...
    for (let i = 0; i < halfLen; i++) {
      ...
      sheetData.push([
        `${yLeft}년`,
        leftData.bal === 0 ? '-' : formatMoney(leftData.bal),
        formatMoney(leftData.interest),
        formatMoney(leftData.principal),
        yRight ? `${yRight}년` : '',
        rightData ? (rightData.bal === 0 ? '-' : formatMoney(rightData.bal)) : '',
        rightData ? formatMoney(rightData.interest) : '',
        rightData ? formatMoney(rightData.principal) : ''
      ]);
    }
...
export function renderLoanScheduleModal(loansList, farmName = '농가') {
  ...
  for (let i = 0; i < halfLen; i++) {
    ...
    yearlyRowsHtml += `
      <tr>
        <td style="font-weight:700; background:rgba(255,255,255,0.03);">${yLeft}년</td>
        ...
        <td style="border-left: 2px solid rgba(255,255,255,0.2); font-weight:700; background:rgba(255,255,255,0.03);">${yRight ? yRight + '년' : ''}</td>
          ...
          <table class="data-table" style="font-size: 13px; text-align: center;">
            ...
            <tbody>
              ${loans.map(l => {
                ...
                return `
                  <tr>
                    <td><span class="badge" style="background:rgba(59,130,246,0.15); color:#60A5FA;">${l.condition}</span></td>
                    <td style="text-align:left; font-weight:800; color:#FFF;">${l.name}</td>
          ...
          <table class="data-table" style="font-size: 13px; text-align: center;">
            <thead>
              <tr style="background: rgba(16, 185, 129, 0.2); color: #A7F3D0;">
                ...
                <th style="width:7%; border-left:2px solid rgba(255,255,255,0.2);">년도</th>
                <th style="text-align:right; width:18%;">연말 잔액</th>
                <th style="text-align:right; color:#FBBF24; width:12.5%;">이자</th>
                <th style="text-align:right; color:#34D399; width:12.5%;">상환액</th>
              </tr>
